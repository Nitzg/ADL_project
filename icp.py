...
class _ICP():
    def __init__(self, e_dim, z_dim, is_cuda):
        self.e_dim = e_dim
        self.z_dim = z_dim
    ...
    def train_epoch(self, z_np, epoch):
        ...
        for i in range(M):
            e = torch.randn(batch_size, self.e_dim)
            if self.is_cuda:
                e = e.cuda()
            y_est = self.netT(e)
        ...
        for i in range(batch_n):
            ...
            e = torch.from_numpy(e_np[indices[rp[idx_np]]]).float()
            z_act = torch.from_numpy(z_np[rp[idx_np]]).float()
            if self.is_cuda:
                e = e.cuda()
                z_act = z_act.cuda()
            z_est = self.netT(e)
...
class ICPTrainer():
    def __init__(self, f_np, d, is_cuda):
        self.f_np = f_np
        self.icp = _ICP(d, f_np.shape[1], is_cuda)
        self.is_cuda = is_cuda

    def train_icp(self, n_epochs):
        uncca_opt_params = OptParams(lr=1e-3, batch_size=1, epochs=n_epochs,
